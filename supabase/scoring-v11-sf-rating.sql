-- ============================================================================
-- V11: Sales-driven scoring — SF Rating is the strongest signal
-- ----------------------------------------------------------------------------
-- V10 problem: SF-tagged "Hot" leads (Sales judgment) scored low in CDP because
-- their engagement happens on Zalo (not tracked) or Sales-side (email_sent,
-- TVV chat) which V10 treats as "weak MDA push, not lead intent".
--
-- V11 principle: If Sales manually tagged Rating=Hot, that IS the intent
-- signal — Sales has direct customer contact and knows more than any tracked
-- event. Rating trumps the automated formula.
-- ============================================================================

-- 1. Add SF metadata columns to dim_lead
ALTER TABLE dim_lead ADD COLUMN IF NOT EXISTS sf_rating TEXT;
ALTER TABLE dim_lead ADD COLUMN IF NOT EXISTS sf_product TEXT;
ALTER TABLE dim_lead ADD COLUMN IF NOT EXISTS sf_status TEXT;
CREATE INDEX IF NOT EXISTS idx_dim_lead_sf_rating ON dim_lead(sf_rating) WHERE sf_rating IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_dim_lead_sf_product ON dim_lead(sf_product) WHERE sf_product IS NOT NULL;

-- 2. Recreate scoring function with SF Rating boost
...
BEGIN
  ...
  computed AS (
    SELECT s.*, 40 +
      -- 🔥 SALES-VERIFIED: SF Rating (highest priority — Sales' judgment)
      (CASE
        WHEN s.sf_rating = 'Hot' THEN 45           -- Sales says Hot → CDP Hot
        WHEN s.sf_rating = 'Warm' THEN 20
        WHEN s.sf_rating = 'Cold' THEN -20
        ELSE 0
      END) +
      ...
      -- 🔥 STRONG: Form submit (boosted from V10 — MDA form = highest intent)
      ...
      -- Silent penalties (softer for SF Hot-tagged leads — they may chat on Zalo we don't track)
      (CASE
        WHEN s.sf_rating = 'Hot' THEN 0   -- SF says Hot → don't penalize silence
        WHEN s.silent_days <= 30 THEN 0
        WHEN s.silent_days <= 90 THEN -20
        WHEN s.silent_days <= 180 THEN -40
        ELSE -60
      END) +
      -- No LEAD engagement penalty (waived if SF-tagged Hot)
      (CASE WHEN NOT s.has_lead_engagement AND s.sf_rating IS DISTINCT FROM 'Hot' THEN -15 ELSE 0 END) AS raw_score
    FROM signals s
  ),
  clamped AS (
    SELECT c.lid, GREATEST(0, LEAST(100, c.raw_score))::INT AS score,
      (SELECT jsonb_agg(reason ORDER BY (reason->>'points')::int DESC) FROM (
        -- SF Rating
        SELECT jsonb_build_object('sign','+','label','⭐ SF: Sales tag Hot','points',45) AS reason WHERE c.sf_rating = 'Hot'
        UNION ALL SELECT jsonb_build_object('sign','+','label','🌡 SF: Sales tag Warm','points',20) WHERE c.sf_rating = 'Warm'
        UNION ALL SELECT jsonb_build_object('sign','-','label','❄️ SF: Sales tag Cold','points',20) WHERE c.sf_rating = 'Cold'
        -- Strong lead signals
        ...
        -- Negatives (silent waived if SF Hot)
        UNION ALL SELECT jsonb_build_object('sign','-','label','😴 Im lặng 30-90 ngày','points',20) WHERE c.silent_days > 30 AND c.silent_days <= 90 AND c.sf_rating IS DISTINCT FROM 'Hot'
        UNION ALL SELECT jsonb_build_object('sign','-','label','😴 Im lặng 90-180 ngày','points',40) WHERE c.silent_days > 90 AND c.silent_days <= 180 AND c.sf_rating IS DISTINCT FROM 'Hot'
        UNION ALL SELECT jsonb_build_object('sign','-','label','😴 Im lặng > 180 ngày','points',60) WHERE c.silent_days > 180 AND c.sf_rating IS DISTINCT FROM 'Hot'
        UNION ALL SELECT jsonb_build_object('sign','-','label','❌ Chưa có LEAD engagement thật','points',15) WHERE NOT c.has_lead_engagement AND c.sf_rating IS DISTINCT FROM 'Hot'
