-- ============================================================================
-- V13: SỬA TIMEOUT — recompute_lead_scores() ngừng cập nhật từ ~2026-07-14
-- ----------------------------------------------------------------------------
-- Triệu chứng (phát hiện 2026-07-18): scored_at mới nhất kẹt ở 2026-07-14,
-- app chạy trên điểm cũ 4 ngày. Gọi recompute_lead_scores() → "canceling
-- statement due to statement timeout" (~10s).
--
-- Nguyên nhân: trong V12, cột lead_source_count được tính bằng SUBQUERY TƯƠNG
-- QUAN — với MỖI lead (49.644 lead) chạy 1 lần, mỗi lần quét bảng
-- fact_touchpoint (117.875 dòng). Tức ~49.6k lần quét → O(lead × touchpoint).
-- Data lớn dần nên vượt ngưỡng statement_timeout khoảng 14/07.
--
-- V13: gom phép đếm đó thành MỘT lần quét fact_touchpoint (GROUP BY lead_id)
-- trong CTE `lead_sources`, rồi LEFT JOIN vào — biến O(lead × touchpoint)
-- thành O(touchpoint). MỌI CÔNG THỨC ĐIỂM GIỮ NGUYÊN 100%, chỉ đổi cách tính
-- lead_source_count nên kết quả điểm không đổi, chỉ chạy nhanh hơn hàng nghìn lần.
-- ============================================================================
...
BEGIN
  ...
  WITH lead_sources AS (
    -- ⚡ Gom 1 lần cho TẤT CẢ lead thay vì subquery tương quan mỗi lead.
    SELECT ft.lead_id, COUNT(DISTINCT ft.source) AS cnt
    FROM fact_touchpoint ft
    WHERE ft.source IN ('smax','salesforce','instantly','web','fanpage')
  ...
  signals AS (
    SELECT l.lead_id AS lid,
      ...
      COALESCE(ls.cnt, 0) AS lead_source_count,
    ...
    FROM dim_lead l
    LEFT JOIN lead_sources ls ON ls.lead_id = l.lead_id
...
-- Index hỗ trợ CTE lead_sources (nếu chưa có) — quét fact_touchpoint theo
-- (source, event_type) rồi gom theo lead_id.
CREATE INDEX IF NOT EXISTS idx_ft_source_event_lead
  ON fact_touchpoint (source, event_type, lead_id);

-- Chạy lại scoring ngay để lấp 4 ngày điểm cũ
