...
import { getCoolLeads, getCoolLeadsCount, getAvailableStages, type LeadListFilter } from "@/lib/supabase/queries";
...
export default async function CoolLeadsPage({
  ...
  const [leads, total, stages] = await Promise.all([
    getCoolLeads(PAGE_SIZE, offset, filter),
    getCoolLeadsCount(filter),
    getAvailableStages(),
  ]);
  ...
  const buildPageUrl = (p: number) => {
    ...
    return `/cool-leads${qs.toString() ? "?" + qs.toString() : ""}`;
  };
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-[1280px] px-8 py-8">
        ...
        <LeadListToolbar leads={leads} total={total} availableStages={stages} exportFilename="cool-leads.csv" />
