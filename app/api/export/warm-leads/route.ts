...
export async function GET() {
  ...
  const { data } = await s
    ...
    .select("hot_score, hot_reasons, dim_lead!inner(full_name, email, phone, company, stage, assignee, email_open_count, email_click_count, email_reply_count, chat_count, chat_staff_count, form_submit_count, source_count, last_engagement_at)")
    .eq("scored_at", today)
    .gte("hot_score", 40)
    .lte("hot_score", 69)
